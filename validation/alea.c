...
double alea(double a, double b, int option) {                // random number (uniform distribution) in  [a b]
    ...
    switch (option) {
        default:
        case 0: // KISS
            r = (double) rand_kiss() / ULONG_MAX;
            //r = (double) rand_kiss() / RAND_MAX_KISS; //Platform differences?
            break;
...
// ===========================================================
int alea_integer(int a, int b, int option) {                // Integer random number in [a b]
    int ir;
    double r;
...
double alea_stable(double alpha, double beta, double nu, double delta,
                   int option) {    // CMS algorithm (J.M. Chambers, C.L. Mallows and B.W. Stuck)
